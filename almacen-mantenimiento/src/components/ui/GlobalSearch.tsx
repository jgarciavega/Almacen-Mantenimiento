    ...
    <div ref={ref} className="relative w-full max-w-md">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 bg-gray-100 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white"
        />
        {query && (
      ...
      {showDropdown && (
        <div className="absolute top-full mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
          {query.trim().length < 2 ? null : total === 0 ? (
            <div className="px-4 py-6 text-center text-gray-400 text-sm">Sin resultados para "{query}"</div>
          ) : (
            <>
              {(data?.productos?.length ?? 0) > 0 && (
                <div>
                  <p className="px-4 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase">Artículos</p>
                  {data!.productos.map((p: any) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-400">{p.sku} · {p.categoria.nombre} · Stock: {p.stockActual} {p.unidad}</p>
                      </div>
                      {p.stockActual <= p.stockMinimo && (
                        <span className="ml-auto text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full shrink-0">Stock bajo</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              {(data?.proveedores?.length ?? 0) > 0 && (
                <div>
                  <p className="px-4 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase">Proveedores</p>
